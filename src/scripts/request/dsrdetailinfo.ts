...
export const REQUEST_get_dsrdetialinfo = (id: any, isloadingview: boolean, ismsg: boolean) => {
  ...
  HTTP_getqueryCaseDsrList(data, isloadingview, ismsg).then(async (res: any) => {
    //console.log(res.total);
    for (let index = 1; index < res.total + 1; index++) {
      const num = index.toString().padStart(4, '0'); //输出序号为“0001“格式
      data.item = { ahdm: id, xh: num };
      await HTTP_getPartyInfoList(data, isloadingview, ismsg).then((res: any) => {
        if (res.code === 0) {
